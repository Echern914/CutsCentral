 ...
 * the request is sent the moment the business exists (createShopAction), and
 * onboarding's last screen can confirm it - or offer "Finish joining" if it
 * didn't go through. It only ever becomes a join request or a link back to
 * /team/link/<key>, and the key is checked against the shape an id can have.
 */
