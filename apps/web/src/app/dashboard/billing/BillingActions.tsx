...
/**
 * "Try it free for 14 days" - the once-only AI trial for a paying Premium
 * shop. Deliberately NOT gold-gradient like the buy buttons: nothing is being
 * charged, and dressing a free trial as a purchase is the kind of thing a
 * barber remembers badly.
 */
export function StartAiTrialButton({ label }: { label: string }) {
  const [pending, startTransition] = useTransition();
  const [error, setError] = useState<string | null>(null);
  return (
    <div className="flex flex-col items-start gap-2">
      <button
        ...
        onClick={() =>
          startTransition(async () => {
            const r = await startAiTrialAction();
            if (r?.error) setError(r.error);
          })
        }
        className="rounded-full border border-gold/50 bg-gold/10 px-6 py-2.5 text-sm font-semibold text-gold transition-colors duration-150 ease-out hover:bg-gold/20 disabled:opacity-50"
      >
