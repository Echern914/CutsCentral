...
export async function generateMetadata({
  ...
  return {
    ...
    // This page is a sub-view of the shop's page now, not the landing surface,
    // so it shouldn't be what a search engine or link preview surfaces on its
    // own - and the URL carries a client's identity token besides.
...
export default async function RewardsPage({
  ...
  return (
    <RewardsClient
      ...
      // With a public page, this renders as its rewards sub-view: back link in
      // the header, no booking CTA (the shop page owns Book). A shop with no
      // page leaves this null and the page keeps standalone behavior.
      shopHref={data.shop.pageSlug ? `/r/${params.magicToken}` : null}
    />
  );
}
