...
export type SkipReason =
   ...
   * occurrence carries the same reason, which reads as the real diagnosis.
   */
...
export async function materializeSeries(
  ...
  for (const occ of occurrences) {
    ...
    try {
      ...
      } else if (err instanceof MirrorNotConfiguredError) {
        logger.error(
          { shopId: input.shopId, seriesId: series.id, index: occ.index, staffId: err.staffId },
          "acuity mirror: ENFORCE with an unmapped chair - recurring occurrence skipped",
        );
