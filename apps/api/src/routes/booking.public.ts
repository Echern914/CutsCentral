 ...
 *
 * Every public surface that reads TargetedSlot rows goes through here so none
 * can drift: the flat payload, the /day chips, and the open-days sweep. The
 * covering time range is computed from the rows themselves, so callers can't
 * under-fetch.
 *
