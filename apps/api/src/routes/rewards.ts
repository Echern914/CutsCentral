...
rewardsRouter.post("/:magicToken/delete", async (req, res) => {
  const ok = await runAsOwner(async (tx) => {
    ...
    // only anonymize it, so the onDelete: Cascade never fires): the client's
    // first name is baked into every sent-nudge body, and any native booking
    // keeps its own name/phone/email snapshot. Scrub both so no identifier
    // survives the deletion (guideline 5.1.1(v)).
