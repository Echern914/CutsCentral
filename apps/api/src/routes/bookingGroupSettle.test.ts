...
const rowsOf = () =>
  prisma.acuityOutboundBlock.findMany({
    ...
    select: { id: true, state: true, acuityBlockId: true, releaseRequested: true, startsAt: true, endsAt: true, acuityCalendarId: true },
  });
...
describe("🔴 a release must not destroy an UNKNOWN", () => {
  ...
  it("proving absence is the only way an UNKNOWN becomes RELEASED with no id", async () => {
    ...
    // 🔴 AND IT HAS TO HAVE SETTLED FIRST. main's release engine (#458) refuses
    // to read an empty listing as proof until the create has had time to show
    // up in it - a lookup landing seconds after the create would read a live
    // block as absent and orphan the very thing it was deleting. Age the create
    // clock past that window; without this the row correctly stays UNKNOWN and
    // waits, which is the safe answer rather than a bug.
    await prisma.$executeRaw`
      UPDATE "AcuityOutboundBlock"
         SET "lastCreateAttemptAt" = now() - interval '30 minutes'
       WHERE "id" = ${unknownBefore.id}`;
    await reconcileShop(shopId);
