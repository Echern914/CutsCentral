...
bookingDashboardRouter.post("/appointments", async (req, res) => {
  ...
  try {
    ...
    // ENFORCING with a chair the mirror cannot protect. The same refusal the
    // public booking page gives, for the same reason: confirming a booking
    // Acuity still shows as free is precisely what the mirror exists to
    // prevent. This used to fall through to a 500, which told the barber
    // nothing and read as the product being broken.
    if (err instanceof MirrorNotConfiguredError) {
      logger.error(
        { shopId, staffId: err.staffId },
        "acuity mirror: ENFORCE with an unmapped chair - dashboard create refused",
      );
...
bookingDashboardRouter.post("/appointments/walk-in", async (req, res) => {
  ...
  const result = await runWithShop(shopId, async (tx) => {
    ...
    // the barber, and that is a worse-of-two-evils taken on purpose.
    ...
    try {
      ...
      if (!(err instanceof MirrorNotConfiguredError)) throw err;
      logger.error(
        { shopId, staffId, appointmentId: appt.id },
        "acuity mirror: ENFORCE with an unmapped chair - walk-in RECORDED ANYWAY and NOT mirrored",
      );
    }
