  ...
  void (async () => {
    await runAsOwner((tx) =>
      tx.emailDelivery.upsert({
        ...
        // A provider event can arrive BEFORE our own write returns; never
        // clobber a status that has already moved on.
        update: {
          kind: input.meta?.kind ?? "unknown",
          shopId: input.meta?.shopId ?? null,
          appointmentId: input.meta?.appointmentId ?? null,
...
export async function applyEmailEvent(params: {
  ...
  try {
    ...
    // An event for a message we never recorded (sent before this shipped, or
    // from another environment sharing the webhook) is not an error.
    ...
    await runAsOwner((tx) =>
      tx.emailDelivery.update({
        ...
        data: {
          ...
          ...(mapped.failureClass ? { failureClass: mapped.failureClass } : {}),
